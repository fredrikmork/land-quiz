...
// Static alpha-2 to alpha-3 mapping (replaces i18n-iso-countries dependency)
...
export const smallCountryCoords: Record<string, [number, number]> = {
...
export const zoomConfig: Record<string, { center: [number, number]; zoom: number }> = {
  // Europe
...
// Reverse lookup: alpha-3 to alpha-2
const alpha3ToAlpha2Map: Record<string, string> = Object.entries(alpha2ToAlpha3Map).reduce((acc, [a2, a3]) => {
  acc[a3] = a2
  return acc
}, {} as Record<string, string>)

export function getAlpha2(alpha3: string): string {
  return alpha3ToAlpha2Map[alpha3] || alpha3
}

// Reverse lookup: numeric code to alpha2
export const numericToAlpha2: Record<string, string> = Object.entries(alpha3ToNumeric).reduce((acc, [alpha3, numeric]) => {
  const alpha2 = alpha3ToAlpha2Map[alpha3] || (alpha3 === 'XKX' ? 'XK' : alpha3)
  acc[numeric] = alpha2
  return acc
}, {} as Record<string, string>)
